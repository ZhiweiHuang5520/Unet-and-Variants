...
parser.add_argument('--experiment', default='compare', help='the path to store sampled images and models' )
...
ex1 = opt.experiment + '1'
mr1 = opt.modelRoot + '1'
ex2 = opt.experiment + '_dilation'
mr2 = opt.modelRoot + '_dilation'

ex3 = opt.experiment + '_spp'
mr3 = opt.modelRoot + '_spp'
...
encoder2 = model.encoderDilation()
decoder2 = model.decoderDilation()

encoder3 = model.encoderSPP()
decoder3 = model.decoderSPP()

encoder1 = model.encoder()
decoder1 = model.decoder()

encoder1.load_state_dict(torch.load('%s/encoder_%d.pth' % (mr1, opt.epochId) ) )
decoder1.load_state_dict(torch.load('%s/decoder_%d.pth' % (mr1, opt.epochId) ) )
encoder1 = encoder1.eval()
decoder1 = decoder1.eval()

encoder2.load_state_dict(torch.load('%s/encoder_%d.pth' % (mr2, opt.epochId) ) )
decoder2.load_state_dict(torch.load('%s/decoder_%d.pth' % (mr2, opt.epochId) ) )
encoder2 = encoder2.eval()
decoder2= decoder2.eval()

encoder3.load_state_dict(torch.load('%s/encoder_%d.pth' % (mr3, opt.epochId) ) )
decoder3.load_state_dict(torch.load('%s/decoder_%d.pth' % (mr3, opt.epochId) ) )
encoder3 = encoder3.eval()
decoder3= decoder3.eval()
...
for i, dataBatch in enumerate(segLoader ):
    ...
    # Test network
    x1, x2, x3, x4, x5 = encoder1(imBatch )
    pred1 = decoder1(imBatch, x1, x2, x3, x4, x5 )
    
    x1, x2, x3, x4, x5 = encoder2(imBatch )
    pred2 = decoder2(imBatch, x1, x2, x3, x4, x5 )
    
    x1, x2, x3, x4, x5 = encoder3(imBatch )
    pred3 = decoder3(imBatch, x1, x2, x3, x4, x5 )
#     # Compute mean IOU
...
#             % ( epoch, iteration, meanAccuracy ) )
    ...
    if iteration % 10 == 0:
        ...
        utils.save_label(-pred1.data, maskBatch.data, colormap, '%s/labelPred1_%d.png' % (opt.experiment, iteration ), nrows=1, ncols=1 )
        utils.save_label(-pred2.data, maskBatch.data, colormap, '%s/labelPred2_%d.png' % (opt.experiment, iteration ), nrows=1, ncols=1 )
        utils.save_label(-pred3.data, maskBatch.data, colormap, '%s/labelPred3_%d.png' % (opt.experiment, iteration ), nrows=1, ncols=1 )
testingLog.close()
# Save the accuracy
